...
import { verfyandresetpasword } from "./Api/Doctor";

const DoctorPassword = ({ match, history }) => {
    ...
    const { mutate, isLoading, status } = useMutation((data) => verfyandresetpasword(data), {
        retry: false,
        onSuccess: (res) => {
            history.replace("/Doctors");
        },
        onError: (err) => SwalAlert('error', 'Error', 'Wrong Password'),
    });
                                        ...
                                        <div className="col-md-12 mr-5">
                                            <div className="form-field">
                                                ...
                                                <div className="passwordWrapper position-relative">
                                                    <input type={showicon ? "password" : "text"} value={existingpassword}
                                                        onChange={(e) => {
                                        ...
                                        <div className="button mt-4">
                                            {isLoading ? <i className="fas fa-spinner fa-pulse"></i> :
                                                <Link
                                                    ...
                                                    } className="btn_darkbluep pl-5 pr-5" data-bs-toggle="modal" data-bs-target=".doctorsPassword_sufl">Update</Link>}
                                        </div>
                                    </div>
                                </form>
                            </div>
