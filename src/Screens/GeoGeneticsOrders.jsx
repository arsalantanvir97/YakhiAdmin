...
import { getGeogeneticsOrders, getOrders } from "./Api/Orders";
import { useQuery } from "react-query";
import Loader from "../components/Loader";

const GeoGeneticsOrders = ({history}) => {
  ...
  const { isFetching, isLoading, data: orders, status: ordstatus, refetch } = useQuery({
    queryKey: ["geogeneticsorders", page, perPage, from, to, status, searchString, sort,],
    queryFn: () => getGeogeneticsOrders(page, perPage, from, status, to, searchString, sort,),
                ...
                <div className="box py-5">
                  ...
                  <div className="row">
                    <ul className="nav nav-tabs tabTop justify-content-center" id="myTab" role="tablist">
                      ...
                      <li className="nav-item flex-grow-0 flex-fill" role="presentation">
                        <button  className="nav-link active" id="genetics-tab" data-bs-toggle="tab" data-bs-target="#genetics" type="button" role="tab" aria-controls="genetics" aria-selected="true">GEO Genetics</button>
                      </li>
                    </ul>
                          ...
                          <div className="col-12">
                            <div className="maain-tabble table-responsive">
                              <table className="table table-bordered zero-configuration">
                                ...
                                <tbody>
                                  {orders?.order?.docs?.length > 0 &&
                                    orders?.order?.docs?.map((orderr, index) => (
                                      <tr>
                                        <td>{index + 1}</td>
